...
fn part1(left_list: &[u32], right_list: &[u32]) -> u32 {
...
fn part2(left_list: &[u32], right_list: &[u32]) -> u32 {
...
fn main() {
    let (left_list, right_list): (Vec<u32>, Vec<u32>) = std::fs::read_to_string("../locations.txt")
        .unwrap()
        .lines()
        .map(|line| {
