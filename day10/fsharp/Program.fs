...
let map: ComplexMap =
          ...
          |> Array.mapi (fun i -> Array.mapi (fun j value -> (Complex(double i, double j), int(Char.GetNumericValue(value)))))
          |> Array.collect id
          |> dict
          
printfn $"%A{map}"

let rec dfs_unique_ends
    (map: ComplexMap)
    (pos: Position)
    (cache: UniqueEndsCache)
    : UniqueEndsCache =
        if cache.ContainsKey(pos) then
            ...
            let directions = [|Complex(0.0, 1.0); Complex(0.0, -1.0); Complex(1.0, 0.0); Complex(-1.0, 0.0)|]
...
let part1 (map: ComplexMap) =
    map |> Seq.map (fun kvp -> if kvp.Value = 0 then (dfs_unique_ends map kvp.Key (UniqueEndsCache())).[kvp.Key].Count else 0)
        |> Seq.sum
...
let part2 (map: ComplexMap) =
    map |> Seq.map (fun kvp -> if kvp.Value = 0 then (dfs_paths map kvp.Key (PathCache())).[kvp.Key] else 0)
        |> Seq.sum
